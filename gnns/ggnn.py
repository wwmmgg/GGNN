...
def sparse_ggnn_layer(node_embeddings: tf.Tensor,
                      ast_adjacency_lists: List[tf.Tensor],
    ...
    * E: number of edges of a given edge type
    ...
    for edge_type_idx, adjacency_list_for_edge_type in enumerate(ast_adjacency_lists):
    ...
    for _ in range(num_timesteps):
        ...
        for edge_type_idx, adjacency_list_for_edge_type in enumerate(ast_adjacency_lists):
            ...
            all_messages_for_edge_type = edge_type_to_message_transformation_layers[edge_type_idx](edge_source_states)  # Shape [E,D]
            messages.append(all_messages_for_edge_type)
            message_source_states.append(edge_source_states)

        messages = tf.concat(messages, axis=0)  # Shape [M, D]
        aggregated_messages = message_aggregation_fn(data=messages,
