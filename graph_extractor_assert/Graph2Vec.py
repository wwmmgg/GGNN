...
from graph_extractor_assert.vec2onehot import vec2onehot
...
def svd(data):
    ...
    return dataMat.round(2).tolist()


if __name__ == "__main__":

    v_path = "../test/assert/node/"
    e_path = "../test/assert/edge/"

    fullnodes_ouptput_tmp = open("../test/assert/fullnodes.json", 'w')

    contract_name = open("../test/assert/name.txt")  # contracts list
    contract_label = open("../test/assert/label.txt")  # contracts label
    ...
    while names:
        ...
        node_vec, graph_edge, var_embedding = construct_vec(edge_list, node_embedding, var_embedding, edge_embedding, edge_encode)
        ...
        fullnode_graph_dict = ({
            ...
            "node_features": svd(fullnodes_feature_list),  
        })
