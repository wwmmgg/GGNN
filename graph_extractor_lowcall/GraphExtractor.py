            ...
            if len(text) > 0 and text != "\n":
                if text.split()[0] == "function" or text.split()[0] == "function()":
                    ...
                    if text.split()[0] != "modifier" and text.split()[0] != "event":
                        ...
                        continue
                elif "pragma solidity" not in text:
                    contract_list.append(text)
        else:
            contract_list.append(text)
            continue
...
def generate_graph(filepath):
    ...
    for i in range(len(allFunctionList)):
        ...
        for j in range(len(allFunctionList[i])):
            ...
            if 'call(' in text or 'callcode('in text or 'delegatecall('in text or 'send('in text:
                callList.append(allFunctionList[i])
                flag += 1
        if flag == 0:
        ...
        for j in range(len(callList[i])):
            ...
            if 'call' in text or 'callcode'in text or 'delegatecall'in text or 'send'in text:
                ...
                for k in range(len(function_limit)):
                    if function_limit[k] in callList[i][0]:
                        limit_count += 1
    ...
    for i in range(len(callList)):
        ...
        for j in range(len(callList[i])):
            ...
            if 'call(' in text or 'callcode('in text or 'delegatecall('in text or 'send('in text:
                callFlag1 += 1
                Vartxor = text.split("(")[1]
                Vartxor = Vartxor.split(")")[0]

            elif 'call' in text or 'callcode'in text or 'delegatecall'in text or 'send'in text:
                callFlag2 += 1
                if 'return' in text:
                ...
                if callFlag1 == 0:
                    ...
                    varFlag += 1
                    break
            if callFlag1 != 0 and callFlag2 == 0:
                if Vartxor != " " or "":
                    if "return" in text and Vartxor in text:
                        node_feature_list.append(["VAR" + str(varFlag), "S" + str(i), 3, 'violation'])
                        edge_list.append(["S" + str(i), "VAR" + str(varFlag), 2, 'RE'])
                        node_list.append("VAR" + str(varFlag))
                        varFlag += 1
                        break
                    elif Vartxor in text:
                        node_feature_list.append(["VAR" + str(varFlag), "S" + str(i), 3, 'warning'])
                        if "assert" in text:
                            edge_list.append(["S" + str(i), "VAR" + str(varFlag), 2, 'AH'])
                        elif "require" in text:
    ...
    if callFlag == 0:
        print(filepath+":Currently, there is no key word lowlevelcall")
...
def printResult(file, node_feature, edge_feature):
    ...
    for i in range(len(node_feature)):
        if 'W' in node_feature[i][0] or 'S' in node_feature[i][0]:
    ...
    nodeOutPath="../test/lowcall/node/" + file+'.sol'
    edgeOutPath = "../test/lowcall/edge/" + file+'.sol'
...
if __name__ == "__main__":
    ...
    inputtxt = open("../test/lowcall/name.txt", 'r')
