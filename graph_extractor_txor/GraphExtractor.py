...
def generate_graph(filepath):
    ...
    for i in range(len(allFunctionList)):
        ...
        for j in range(len(allFunctionList[i])):
            ...
            if 'tx.origin' in text:
                txorList.append(allFunctionList[i])
                flag += 1
        if flag == 0:
    ...
    for i in range(len(txorList)):
        node_list.append("S" + str(i))
        node_list.append("W" + str(i))
        txorFlag += 1

        for j in range(len(txorList[i])):
            text = txorList[i][j]
            ...
            if 'tx.origin' in text:
                ...
                for k in range(len(function_limit)):
                    if function_limit[k] in txorList[i][0]:
                        limit_count += 1
    ...
    for i in range(len(txorList)):
        ...
        for j in range(len(txorList[i])):
            text = txorList[i][j]
            if 'tx.origin' in text and "=" in text:
                txorFlag1 += 1
                Vartxor = text.split("=")[0]
            elif 'tx.origin' in text:
                txorFlag2 += 1
                if 'return' in text:
                ...
                if txorFlag1 == 0:
                    ...
                    varFlag += 1
                    break
            if txorFlag1 != 0 and txorFlag2 == 0:
                if Vartxor != " " or "":
    ...
    if txorFlag == 0:
        print(filepath+":Currently, there is no key word tx.origin")
...
def printResult(file, node_feature, edge_feature):
    ...
    nodeOutPath="../test/test_txor/node/" + file
    edgeOutPath = "../test/test_txor/edge/" + file


    # f_node = open(nodeOutPath, 'a')
    # for i in range(len(node_feature)):
    #     #print(file)
    #     #print(type(node_feature[i][2]))
    #     if isinstance (node_feature[i][2],list):########################################################################
    #         if len(node_feature[i][2])==1:
    #             node_feature[i][2]=node_feature[i][2][0]
    #     else:#错误！！！！！########################################################################################################
...
if __name__ == "__main__":
    ...
    inputtxt = open("../test/test_txor/name.txt", 'r')
