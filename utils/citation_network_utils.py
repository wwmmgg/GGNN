...
def load_data(directory: str, dataset_str: str):
    ...
    names = ['x', 'y', 'allx', 'ally', 'graph11', 'graph21']
    objects = []
    ...
    x, y, allx, ally, graph11,graph21  = tuple(objects)
    ...
    idx_val = range(len(y), len(y) + 3600)

    train_mask = sample_mask(idx_train, labels.shape[0])
    val_mask = sample_mask(idx_val, labels.shape[0])
    test_mask = sample_mask(range(0), labels.shape[0])
    ...
    return graph11,graph21,features, y_train, y_val, y_test, train_mask, val_mask, test_mask
